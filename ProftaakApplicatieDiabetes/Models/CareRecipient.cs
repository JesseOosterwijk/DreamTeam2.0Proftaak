...
namespace Models
{
    public class CareRecipient : User
    {
        ...
        public CareRecipient(int userId, int userBSN, AccountType accountType, string firstName, string lastName, string email, string password, string address, string residence, Gender gender, DateTime dateOfBirth, bool status) : base(userId, userBSN, accountType, firstName, lastName, email, password, address, residence, gender, dateOfBirth, status)
        {
        }
    }
}
